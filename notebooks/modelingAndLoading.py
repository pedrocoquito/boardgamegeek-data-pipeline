...
df_games = df.select('game_id', 'name', 'year_published', 'min_players', 'max_players', 'play_time', 'min_age').distinct()
...
df_mechanics = df.select('game_id', explode(split('mechanics', ', ')).alias('mechanic_name')).distinct()
...
df_domains = df.select('game_id', explode(split('domains', ', ')).alias('domain_name')).distinct()
